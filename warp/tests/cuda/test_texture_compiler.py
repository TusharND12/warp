...
fetches at runtime when targeting ``sm_89``. The selected scalar fetch returns
zero, even though the unselected vector branch is never executed. Direct
texture-sampling kernels do not reproduce the bug. The kernels below retain
the smallest known surrounding dataflow that does: a runtime selector, two
fetches, interpolation, and an out-of-bounds distance adjustment.
...
the compiler regression this module is meant to detect.
"""
